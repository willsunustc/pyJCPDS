...
class Main(QMainWindow):
	...
	def initUI(self):
		...
		self.jlabel = QLabel('Information of jcpds file')
		self.clabel = QLabel('Information of cif file')
		self.tlabel = QLabel('Information of jcpds converted from cif')
		self.jedit = QTextEdit()
		self.cedit = QTextEdit()
		self.tedit = QTextEdit()

		self.tbutton = QPushButton(QIcon('./resources/icons/save.png'), 'Save as jcpds', self)
		...
		self.vboxp.addWidget(self.canvas)
		self.plot.setLayout(self.vboxp)

		self.setCentralWidget(self.tabWidget)

		impjAct = QAction(QIcon('./resources/icons/importj.png'), 'Import jcpds', self)
		impjAct.setShortcut('Ctrl+J')
		impjAct.triggered.connect(self.loadandreadj)

		impcAct = QAction(QIcon('./resources/icons/importc.png'), 'Import cif', self)
		impcAct.setShortcut('Ctrl+F')
		impcAct.triggered.connect(self.loadandreadc)

		trsAct = QAction(QIcon('./resources/icons/convert.png'), 'cif to jcpds', self)
		trsAct.setShortcut('Ctrl+T')
		trsAct.triggered.connect(self.ciftojcpds)

		plotjAct = QAction(QIcon('./resources/icons/plotj.png'), 'Plot jcpds', self)
		plotjAct.setShortcut('Ctrl+P')
		plotjAct.triggered.connect(self.plot_jcpds)

		plotcAct = QAction(QIcon('./resources/icons/plotc.png'), 'Plot cif', self)
		plotcAct.setShortcut('Ctrl+O')
		plotcAct.triggered.connect(self.plot_cif)

		exitAct = QAction(QIcon('./resources/icons/exit.png'), 'Exit', self)
	...
	def loadandreadj(self):
		...
		fname = QFileDialog.getOpenFileName(self, 'Import jcpds', './resources/jcpds_files', '*.jcpds')
	...
	def loadandreadc(self):
		...
		fname = QFileDialog.getOpenFileName(self, 'Import cif', './resources/cif_files', '*.cif')
	...
	def ciftojcpds(self):
		'''
		import a cif file, convert it to jcpds, display information and
		...
		fname = QFileDialog.getOpenFileName(self, 'Choose cif', './resources/cif_files', '*.cif')
		...
		self.tedit.append(5*'-' + 'converted jcpds information' + 5*'-')
	...
	def savejcpds(self):
		#save converted jcpds file
		fname = QFileDialog.getSaveFileName(self, 'Save as jcpds', './converted_jcpds_from_cif', '*.jcpds')
	...
	# Plot the vlines for JCPDS files
	def plot_jcpds(self):
		#change tab and page
		self.tabWidget.setCurrentWidget(self.plot)

		#import jcpds file
		pattern = JCPDS_extend(filename = self.jcpds_filename)

		#calculate the pattern
		pattern.cal_dsp(pressure = self.pressurej)
		pattern.get_DiffractionLines()
		tth, inten = pattern.get_tthVSint(wavelength = self.wavelengthj)

		# show tooltips
		def onpick3(event):
			ind = event.ind
			print('onpick3 scatter:', ind, np.take(tth, ind), np.take(inten,ind))

		#plot
		# self.drawing.plot(tth, inten, color = 'r', label = 'jcpds')
		self.drawing.vlines(tth, 0., inten, color = 'r', label='jcpds', picker=True)
		self.drawing.set_xlabel('two theta')
		self.drawing.set_ylabel('intensity')
...
class Paradialogj(QDialog):
	...
	def __init__(self, parent=None):
		...
		self.setWindowIcon(QIcon('./resources/icons/set.png'))
